...
2. **无尽酷跑 (`runner`)**：
   - 障碍物从单调的 2 种大幅扩展为 6 种完全不同的挑战：地面单尖刺、连环双尖刺、滚动齿轮、低空无人机、浮空激光雷、高石墙。
   - 新增自定义弹窗：速度等级、障碍物密度、6 种障碍物类型自由开关、玩家/地面尖刺调色盘自定义。
3. **吃豆人迷宫 (`pacman`)**：
   - 彻底修复“按一下向上一直移动不停止”的键盘释放未监听 bug，改为精准的按键按下移动、松开即止判定。
4. **宝石消除 (`match3`)**：
